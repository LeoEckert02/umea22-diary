<script>
    ...
    import { lightTheme } from '../stores/stores'

    let theme = false;

    function updateLocalStorageTheme() {
        if (theme) {
            lightTheme.set('corporate');
        } else {
            lightTheme.set('dracula')
        }
    ...
    onMount(async () => {
        const localTheme = localStorage.getItem("lightTheme");
        if (localTheme) {
            lightTheme.set(localTheme);
            if (localTheme === 'corporate' ) {
                theme = false;
            } else {
                theme = true;
            }
...
<label class="swap swap-rotate mt-1 md:mt-0 opacity-40">
  
    <!-- this hidden checkbox controls the state -->
    <input type="checkbox" bind:checked={theme} on:click={updateLocalStorageTheme}/>
